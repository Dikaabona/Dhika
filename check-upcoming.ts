...
async function check() {
  ...
  const { data: schedules, error } = await supabase.from('schedules').select('*').gte('date', today).order('date', { ascending: true }).limit(5);
  console.log("Upcoming Schedules:", schedules);
  if (error) console.error(error);

  const { data: assignments, error2 } = await supabase.from('shift_assignments').select('*').gte('date', today).order('date', { ascending: true }).limit(5);
