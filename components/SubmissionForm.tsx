    ...
    <div className="max-w-2xl mx-auto bg-white p-6 sm:p-10 rounded-[32px] sm:rounded-[48px] shadow-sm border border-slate-50">
      <div className="flex items-center gap-4 sm:gap-5 mb-8 sm:mb-10">
        <div className="bg-[#FFC000] p-3 sm:p-4 rounded-xl sm:rounded-2xl text-slate-900 shadow-lg shadow-amber-200/50">
          <Icons.Megaphone className="w-6 h-6 sm:w-8 sm:h-8" />
        </div>
        <div className="flex flex-col">
          <h2 className="text-xl sm:text-3xl font-black text-[#0f172a] tracking-tight leading-none">PUSAT PENGAJUAN</h2>
          <p className="text-[9px] sm:text-[11px] text-slate-400 font-bold tracking-[0.2em] sm:tracking-[0.3em] uppercase mt-1.5 sm:mt-2">CUTI • IZIN • SAKIT • LEMBUR</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {(['Cuti', 'Izin', 'Sakit', 'Lembur'] as AttendanceStatus[]).map(s => (
              <button 
                ...
                type="button"
                onClick={() => setStatus(s)}
                className={`py-3 sm:py-5 rounded-[18px] sm:rounded-[24px] font-bold text-[9px] sm:text-[11px] uppercase tracking-widest transition-all shadow-sm ${
                  status === s 
        ...
        {status === 'Lembur' ? (
          <div className="space-y-5 sm:space-y-6 animate-in fade-in slide-in-from-top-2">
             <div className="space-y-2 sm:space-y-3">
                ...
                <input 
                  ...
                  className="w-full bg-[#f8fafc] px-6 py-4 sm:px-8 sm:py-6 rounded-[24px] sm:rounded-[32px] border border-slate-100 text-[11px] sm:text-sm font-black text-[#0f172a] outline-none focus:ring-2 focus:ring-[#FFC000] transition-all shadow-inner"
                />
             </div>
        ...
        {status === 'Sakit' && (
          <div className="space-y-2 sm:space-y-3">
            ...
            <label 
              htmlFor="doc-upload" 
              className={`flex flex-col items-center justify-center p-6 sm:p-10 border-2 border-dashed rounded-[24px] sm:rounded-[32px] cursor-pointer transition-all ${docBase64 ? 'bg-emerald-50 border-emerald-300' : 'bg-[#f8fafc] border-slate-200 hover:border-[#FFC000]'}`}
            >
              <div className={docBase64 ? 'text-emerald-600' : 'text-slate-400'}>
                <Icons.Upload className="w-6 h-6 sm:w-8 sm:h-8" />
              </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <textarea 
            ...
            className="w-full px-6 py-4 sm:px-8 sm:py-6 bg-[#f8fafc] border border-slate-100 rounded-[24px] sm:rounded-[32px] text-[11px] sm:text-sm font-medium text-slate-900 outline-none focus:ring-2 focus:ring-[#FFC000] resize-none shadow-inner"
          />
        </div>
        ...
        <button 
          disabled={isLoading} 
          type="submit" 
          className="w-full bg-[#0f172a] hover:bg-black text-[#FFC000] py-4 sm:py-7 rounded-[28px] sm:rounded-[40px] font-bold text-[10px] sm:text-[11px] uppercase tracking-[0.2em] sm:tracking-[0.3em] shadow-2xl transition-all disabled:opacity-50 active:scale-[0.98] mt-2 sm:mt-4"
        >
